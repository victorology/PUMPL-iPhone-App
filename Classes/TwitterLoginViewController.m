...
- (void)requestFinished:(ASIHTTPRequest *)request
{
	...
	if(code == 0)
	{
		if([[[responseDic valueForKey:@"value"] valueForKey:@"is_twitter_connected"] boolValue])
		{
			...
			UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:NSLocalizedString(@"CongratulationsKey", @"") message:@"You have successfully logged into your twitter account." delegate:self cancelButtonTitle:NSLocalizedString(@"OkKey", @"") otherButtonTitles:nil];
			alertView.tag = 1;
			[alertView show];
			[alertView release];
		}
		...
		{
			UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:nil message:@"Could not log into twitter screen" delegate:nil cancelButtonTitle:NSLocalizedString(@"OkKey", @"") otherButtonTitles:nil];
			[alertView show];
			[alertView release];
		}
	}
