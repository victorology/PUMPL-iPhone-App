...
@implementation RenderTexture (Photochrom)

+(void) loadPhotochromProgram 
{
    if(!texColorMap)
    {
        texColorMap = [[GLTexture alloc] initWithImage:[UIImage imageNamed:@"photochrom_colormap.png"]];
    }
    
    if(!program)
    {
        program = [[GLProgram alloc] initWithVertexShaderFilename:@"PhotochromShader" fragmentShaderFilename:@"PhotochromShader"];
...
{    
    [self renderBegin];
    [self drawPhotochromInRect:CGRectMake(-1, 1, 2, -2)];
    [self renderEnd];
}
