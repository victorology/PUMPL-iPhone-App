...
#define kPaddingBetweenTitleAndToolbar 10
...
- (void)trashPhoto;
- (void)exportPhoto;
- (void)updatePhotoTitleAtIndex:(NSInteger)index;
@end
...
- (void)loadView 
{
   ...
   [space release];
	...
	titleLabel_ = [[UILabel alloc] initWithFrame:CGRectMake(0, 200, 320, 40)];
	titleLabel_.textColor = [UIColor whiteColor];
	titleLabel_.backgroundColor = [UIColor clearColor];
	titleLabel_.numberOfLines = 10;
	titleLabel_.hidden = YES;
	titleLabel_.textAlignment = UITextAlignmentCenter;
	titleLabel_.font = [UIFont boldSystemFontOfSize:kTitleLabelFontSize];
...
- (void)setCurrentIndex:(NSInteger)newIndex
{
   ...
   [self setTitleWithCurrentPhotoIndex];
	[self updatePhotoTitleAtIndex:newIndex];
   [self toggleNavButtons];
}
...
#pragma mark -
#pragma mark Photo Title Label Support

- (void)updatePhotoTitleAtIndex:(NSInteger)index
{
	if([dataSource_ respondsToSelector:@selector(titleAtIndex:)])
	{
		NSString *photoTitle = [dataSource_ titleAtIndex:index];
